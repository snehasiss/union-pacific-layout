...
def test_import_dc_locomotive_translates_decoder_and_address():
    locos = LocoImport.import_file(IMPORT_DIRECTORY / "steam.csv")
    loco = next(loco for loco in locos if loco.reporting_mark == "UP" and loco.road_number == "5090")
...
def test_import_diesel_file_1():
    locos = LocoImport.import_file(IMPORT_DIRECTORY / "diesel.csv")
...
def test_import_turbine():
    locos = LocoImport.import_file(IMPORT_DIRECTORY / "diesel.csv")
    turbine = next(loco for loco in locos if loco.loco_type == LocoType.TURBINE)
...
def test_reporting_mark_is_normalised_to_uppercase():
    loco = LocoImport.import_file(IMPORT_DIRECTORY / "steam.csv")[0]
    assert loco.reporting_mark == "UP"
...
def test_invalid_boolean_raises():
    row = {"dcc": "maybe", "light": "yes", "sound": "no", "smoke": "no", "decoder": "dc", "address": "0"}
...
def test_import_diesel_file():
    locos = LocoImport.import_file(IMPORT_DIRECTORY / "diesel.csv")
    assert len(locos) == 122
    assert all(loco.loco_type in {LocoType.DIESEL, LocoType.TURBINE} for loco in locos)
    assert all(loco.control.address == 0 for loco in locos if loco.control.type == ControlType.DC)
    assert all(loco.control.address >= 1 for loco in locos if loco.control.type == ControlType.DCC)
