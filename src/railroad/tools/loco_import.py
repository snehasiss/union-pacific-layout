...
class LocoImport:
    ...
    def import_all(cls) -> list[Loco]:
        ...
        return cls.import_file(cls.STEAM_FILE) + cls.import_file(cls.DIESEL_FILE)

    @classmethod
    def import_and_save(cls, dao: LocoDAO, path: str | Path) -> list[Loco]:
    ...
    @staticmethod
    def _normalise_rows(reader: csv.DictReader) -> list[dict[str, str]]:
    ...
    def _create_control(row: dict[str, str]) -> Control:
        ...
        control_type = ControlType.DCC if is_dcc else ControlType.DC
    ...
    def _boolean(value: str) -> bool:
        ...
        if value == "no":
            return False
        raise ValueError(f"Invalid boolean value '{value}'. Expected yes or no.")
